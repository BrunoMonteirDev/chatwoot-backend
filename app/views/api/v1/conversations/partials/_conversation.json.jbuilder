...
latest_message = conversation.list_data_preloaded? ? conversation.list_latest_message : conversation.messages.where(account_id: conversation.account_id).last
if latest_message.blank?
  json.messages []
else
  json.messages [
    latest_message.try(:push_event_data)
  ]
end
...
json.unread_count conversation.list_data_preloaded? ? conversation.list_unread_count : conversation.unread_incoming_messages.count
last_chat_message = conversation.list_data_preloaded? ? conversation.list_latest_chat_message : conversation.messages.where(account_id: conversation.account_id).non_activity_messages.first
json.last_non_activity_message last_chat_message.try(:push_event_data)
