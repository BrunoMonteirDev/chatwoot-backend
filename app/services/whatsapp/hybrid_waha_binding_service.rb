class Whatsapp::HybridWahaBindingService
  ...
  def bind!
    raise ArgumentError, 'Only official WhatsApp channels can bind WAHA' unless channel.is_a?(Channel::Whatsapp) && channel.provider == 'whatsapp_cloud'
    ...
    channel.assign_attributes(hybrid_waha_session: session, hybrid_enabled: true)
  ...
  def unbind!
    ...
    channel.assign_attributes(hybrid_waha_session: nil)
    channel.save!(validate: false)
    channel
  end
