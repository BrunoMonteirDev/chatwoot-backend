class Whatsapp::HybridRouter
  ...
  def send_waha(decision)
    ...
    operation = attachment ? (%w[image audio video].include?(attachment.file_type) ? attachment.file_type : 'document') : :text
    result = Whatsapp::HybridWahaBridgeClient.new(channel: channel).dispatch(
      ...
      payload: { remote_jid: remote_jid, content: message.content, reply_to: reply_target_provider_key,
                 attachment: attachment && { url: attachment.file_url, fileName: attachment.file.filename.to_s, contentType: attachment.file.content_type } }
    )
